...
const DataDetailsModal = ({ handleCloseModal }: DataDetailsProps) => {
  const selectedData = useAppSelector(state => state.modal.selectedPost);

  const handleShare = async () => {
    const url = window.location.href;
    const title = selectedData?.title || 'Post';
        ...
        <div className='flex justify-end'>
          <div className='flex flex-col '>
            ...
            <ul className='flex flex-col text-sm'>
              <li className='flex font-medium mt-3'>
                <span>{selectedData?.category?.name}</span>
              </li>
            </ul>
          </div>
        </div>
