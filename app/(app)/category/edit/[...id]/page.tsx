...
export default async function Edit({ params }: EditProps) {
    ...
    const initialCategory = await serverClient.getCategoryById(id);
        ...
        <>
            <section className='w-full space-y-4 px-4 md:px-4 lg:mx-auto lg:max-w-4xl lg:px-10 xl:max-w-7xl'>
                <div className='flex items-center justify-between'>
                    <h3 className='md:text-2xl'>Edit: {initialCategory?.name}</h3>
                </div>
                <div className='flex flex-col justify-center pt-2'>
                    <PageForm initialCategory={initialCategory} categoryId={id} />
                </div>
            </section>
        </>
    );
