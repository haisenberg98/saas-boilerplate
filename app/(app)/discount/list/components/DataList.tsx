...
const DataList = () => {
    const { data: listData, refetch } = trpc.getDiscountCodes.useQuery(undefined, {
        refetchOnMount: false, //prevent refetching on mount
        refetchOnReconnect: false //prevent refetching on reconnect
    });

    //trpc
    const deleteData = trpc.deleteDiscountCode.useMutation({
        onError: (error) => {
    ...
    return (
        <section className='w-full space-y-4 px-4 lg:mx-auto lg:max-w-4xl xl:max-w-7xl'>
            <div className='flex items-center justify-between'>
                <h3 className='md:text-2xl'>All discounts</h3>
                        ...
                        <tbody>
                            {listData.map((item, index) => (
                                <tr
                                    ...
                                    <td className='py-2'>{item.discountValue}</td>
                                    <td className='py-2'>{item.usageCount}</td>
                                    <td className='py-2'>{item.maxUsage}</td>
