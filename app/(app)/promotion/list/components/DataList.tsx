...
const DataList = () => {
    const { data: listData, refetch } = trpc.getAllPromotions.useQuery(undefined, {
        refetchOnMount: false, //prevent refetching on mount
        refetchOnReconnect: false //prevent refetching on reconnect
    });

    //trpc
    const deleteData = trpc.deletePromotion.useMutation({
        onError: (error) => {
    ...
    return (
        <section className='w-full space-y-4 px-4 lg:mx-auto lg:max-w-4xl xl:max-w-7xl'>
            <div className='flex items-center justify-between'>
                <h3 className='md:text-2xl'>All promotions</h3>
                            ...
                            {listData.map((item, index) => (
                                <tr
                                    ...
                                    <td className='flex space-x-4 py-2'>
                                        <Link href={`/promotion/edit/${item.id}`}>
                                            <FaPencilAlt size={20} />
                                        </Link>
